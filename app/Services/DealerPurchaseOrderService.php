...
class DealerPurchaseOrderService
{
    public function create(array $data, int $userId): DealerPurchaseOrder
    {
        return DB::transaction(function () use ($data, $userId) {
            ...
            $this->addHistory($po, 'draft', 'Purchase order created.', $userId);
    ...
    public function approve(
        ...
        return DB::transaction(function () use ($po, $userId) {
            if ($po->status !== 'pending_approval') {
                throw new \Exception('Only pending approval PO can be approved.');
            }
            ...
            $this->addHistory($po, 'approved', 'Purchase order approved.', $userId);
    ...
    public function reject(
        ...
        return DB::transaction(function () use ($po, $userId, $note) {
            if ($po->status !== 'pending_approval') {
                throw new \Exception('Only pending approval PO can be rejected.');
            }
    ...
    public function cancel(
        ...
        return DB::transaction(function () use ($po, $userId) {
            if (in_array($po->status, ['fully_delivered', 'cancelled'])) {
            ...
            $this->addHistory($po, 'cancelled', 'Purchase order cancelled.', $userId);
    ...
    private function calculateTotals(array $items): array
    {
        ...
        foreach ($items as $item) {
            ...
            $lineSubtotal = max(0, $gross - $discount);
    ...
    private function createItems(
        ...
        foreach ($items as $item) {
            ...
            $tax       = (float) ($item['tax_amount'] ?? 0);
            $gross     = $quantity * $unitPrice;
            $total     = max(0, $gross - $discount) + $tax;
    ...
    private function addHistory(
        ...
        DealerPurchaseOrderHistory::create([
            'dealer_purchase_order_id' => $po->id,
            'status'                   => $status,
            'note'                     => $note,
            'created_by'               => $userId,
        ]);
    ...
    {
        $prefix  = 'DPO-' . now()->format('Ym') . '-';
        $lastPo  = DealerPurchaseOrder::where('po_number', 'like', $prefix . '%')
            ->orderByDesc('id')
            ->first();
