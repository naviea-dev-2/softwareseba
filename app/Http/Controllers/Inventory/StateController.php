...
class StateController extends Controller
{
    ...
    {
        if(can_p('state.index') == false){
            return redirect()->route('dashboard');
        }
       
        return view ('Inventory.location.state.manage');
    }
    function ajaxState(Request $request){
        ...
        {
           $i = $start == 0 ? 1 : $start+1;
           $p_edit = can_p('state.edit');
           $p_delete = can_p('state.delete');
