...
use App\Imports\ValidateProductImport;
...
class BulkImportController extends Controller
{
     ...
     public function index()
    {
        ...
        $template = new TemplateProductExport('xlsx',auth()->user()->business->business_type_id);
        $data['headings'] = $template->headings();
        $data['data'] = $template->collection();
        $data['rules'] = $template->rules();
        // dd($data);
        // ValidateProductImport
        return view('Inventory.product.bulk-import',$data);
    }
    public function postImport(Request $request){
       // dd($request);
        $file = $request->file('file');
        $validateProductImport= new ValidateProductImport($request,auth()->user()->business->business_type_id,auth()->user()->business->id);
        ...
        $productImport = new ProductImport($request,auth()->user()->business->business_type_id,auth()->user()->business->id);
     ...
     public function downloadTemplate(Request $request)
    {
        ...
        return (new TemplateProductExport($extension,auth()->user()->business->business_type_id))->download($fileName, $writeType, $contentType);
    }
}
