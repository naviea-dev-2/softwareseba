...
class UnitController extends Controller
{
    ...
    {
        return view ('admin.Inventory.unit.manage');
    }
    function ajaxUnit(Request $request){
        $columns = array(
           0 => 'units.id',
           1 => 'units.name',
           2 => 'units.symbol',
       ...
       $products = Unit::leftJoin('businesses','businesses.id','units.business_id');
       if(!empty($search))
       {
           $products = $products->where("units.name","LIKE","%{$search}%")
           ->orWhere("businesses.business_name","LIKE","%{$search}%");
       }
       $products = $products->select('units.*','businesses.business_name')->offset($start)->limit($limit)->orderBy($order,$dir)->get();
       $data = array();
       if(!empty($products))
       {
            $i = $start == 0 ? 1 : $start+1;
           ...
           foreach($products as $product)
           {
               ...
               $nestedData['symbol'] = $product->symbol;
    ...
    function select2unit(Request $request){
        ...
        $units = Unit::select('id', 'name')->where('business_id',$request->business_id)->where("name", "LIKE", "%$request->value%")->get();
       $data[] = ['id' =>"", 'text' =>"Select"];
    ...
    public function store(Request $request)
    {
        if($request->id==0){
            ...
            $validator = Validator::make($request->all(),[
                ...
                'name'=>[
                    'required',
                    Rule::unique('units')->where(function ($query) use ($request,$id) {
    ...
    public function edit(Request $request)
    {
        ...
        return response()->json(['html' => $html,'id'=>$data->id,'symbol'=>$data->symbol,'name'=>$data->name,'business_id'=>$data->business_id ,'business_name'=>$data->business?->business_name]);
    }
