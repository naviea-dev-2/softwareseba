...
class ManufatureController extends Controller
{
    ...
    {
        return view ('admin.Inventory.manufacture.manage');
    }
    function ajaxManufacture(Request $request){
        $columns = array(
           0 => 'manufatures.id',
           1 => 'manufatures.name',
           2 => 'businesses.business_name',
       );
       $totalData = Manufature::count();
       ...
       $products = Manufature::leftJoin('businesses','businesses.id','manufatures.business_id');
       if(!empty($search))
       {
           $products = $products->where("manufatures.name","LIKE","%{$search}%")
           ->orWhere("businesses.business_name","LIKE","%{$search}%");
       }
       $products = $products->select('manufatures.*','businesses.business_name')->offset($start)->limit($limit)->orderBy($order,$dir)->get();
       $data = array();
       if(!empty($products))
       {
            $i = $start == 0 ? 1 : $start+1;
     ...
     function select2ManufaturerList(Request $request){
        $manufactures = Manufature::select('id', 'name')->where('business_id',$request->business_id)->where("name", "LIKE", "%$request->value%")->get();
     ...
     public function store(Request $request)
    {
        if($request->id==0){
            $validator = Validator::make($request->all(),[
                ...
                'name'=>[
                    'required',
                    Rule::unique('manufatures')->where(function ($query) use($request){
            ...
            $validator = Validator::make($request->all(),[
                ...
                'name'=>[
                    'required',
                    Rule::unique('manufatures')->where(function ($query) use ($request,$id) {
    ...
    public function destroy(Request $request,$id)
    {
        ...
        try{
            ...
            return redirect()->route('admin.manufacture.index')->with($notification);
