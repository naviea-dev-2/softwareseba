...
class CategoryController extends Controller
{
    ...
    {
        return view ('admin.Inventory.category.manage');
    }
    function ajaxCategory(Request $request){
        $columns = array(
           0 => 'categories.id',
           1 => 'categories.name',
       ...
       $products = Category::leftJoin('businesses','businesses.id','categories.business_id');
       if(!empty($search))
       {
           $products = $products->where("categories.name","LIKE","%{$search}%")
           ->orWhere("businesses.business_name","LIKE","%{$search}%");
       }
       $products = $products->select('categories.*','businesses.business_name')->offset($start)->limit($limit)->orderBy($order,$dir)->get();
       $data = array();
       if(!empty($products))
       {
            $i = $start == 0 ? 1 : $start+1;
    ...
    function select2CategoryList(Request $request){
        $categories = Category::select('id', 'name')->where('business_id',$request->business_id)->where("name", "LIKE", "%$request->value%")->get();
