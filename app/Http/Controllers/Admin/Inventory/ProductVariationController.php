...
class ProductVariationController extends Controller
{
    function addNewProductAttribute(Request $request){
        ...
        $attribute_sets = AttributeSet::query();
        if($request->b_id){
            $attribute_sets = $attribute_sets->where('business_id',$request->b_id);
        }
        $attribute_sets = $attribute_sets->orderBy('order','asc')->get();
        $product_id = $request->input('id');
        //dd($attribute_sets);
        return view("admin.Inventory.product.product_attribute_sets",compact("attribute_sets","product_id"));
    }
