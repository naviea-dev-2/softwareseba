...
class HrController extends Controller
{
    ...
    public function editDepartment($id){
        ...
        return view('Hr.department.editdepartment',compact('department'));
    }
    ...
    public function editDesignation($id){
        ...
        return view('Hr.designation.editdesignation',compact('designation','department'));
    ...
    public function updateDesignation(Request $request, $id)
    {
        if(can_p('editDesignation') == false){
         ...
         $this->validate($request,[
            'name'=>[
                'required',
                Rule::unique('designations')->where(function ($query) use ($id) {
                    return $query->where('id', '!=', $id)
                        ->where('business_id', auth()->user()->business->id);
                }),
            ],
    ...
    function ajaxEmployee(Request $request){
        ...
        if(!empty($datalist))
        {
            ...
            foreach ($datalist as $data_v) {
                ...
                $nestedData['options']='';
                if($p_edit){
                    $nestedData['options'] .= '<a href="'.url('editEmployee') .'/'. $data_v->id.'" class="btn btn-primary "><i class="bx bx-edit"></i></a>';
                }
               
                if($p_delete){
                    $nestedData['options'] .= '<a del_data="'. $data_v->id .'" class="m-1 del_hr_data btn btn-danger "><i class="bx bx-trash"></i></a>';
                }
        ...
        try{
           ...
           // }
            ...
            $notification=array(
                ...
                return redirect('/allEmployee')->with($notification);
        }catch(\Exception $e){
            DB::rollBack();
            // dd($e->getMessage());
             $notification=array(
          ...
          return redirect('/allEmployee')->withInput($request->all())->with($notification);
        }
    ...
    function select2Employee(Request $request){
        $datas = Employee::select('id', 'employee_name')->where('designation_id',$request->desig_id)->where("employee_name", "LIKE", "%$request->value%")->get();
        $res = [];
        foreach ($datas as $data) {
...
//  dd($absent);
            ...
            return redirect('/manageSalary')->with($notification);
    ...
    public function salarySlip(Request $request){
        ...
        ->first();
            ...
            $data['payments'] = Payment::where('relation_id',$SalarySheet->id)->where('relation_type',"Salary Payment")->get();
        ...
        if(!empty($datalist))
        {
            ...
            foreach ($datalist as $data_v) {
                ...
                if($data_v->payment_status != 2){
                    if($p_edit){
                        $nestedData['options'] .= ' <a class="btn btn-primary mb-1" href="javascript:void(0)" data-token="'.csrf_token().'" id="salarySheetEdit" data-id="'.$data_v->id.'" data-due="'. round($data_v->due_amount,2) .'" data-bs-toggle="modal" data-bs-target="#updateModal"> <i class="bx bx-money"></i></a>';
                    }
                }
                if($p_slip){
                    $nestedData['options'] .= '<a class="btn btn-info salarySlipFetch mb-1" href="javascript:void(0)" data-token="'.csrf_token().'" data-id="'.$data_v->id.'" data-bs-toggle="modal" data-bs-target="#salarySlip"><i class="bx bx-show"></i></a>';
                }
                if($p_delete){
                    $nestedData['options'] .= '<a title="Delete" class="del_hr_data btn btn-danger" data-token="'.csrf_token().'" data-id="'.$data_v->id.'"><i class="bx bx-trash"></i></a>';
                }
