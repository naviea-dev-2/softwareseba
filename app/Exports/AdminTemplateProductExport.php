...
class AdminTemplateProductExport implements
    FromCollection,
    WithHeadings,
    WithEvents,
    ...
    ShouldAutoSize
{
    ...
    protected int $business_type_id;

    protected Collection $business_type;
   ...
   protected Collection $attributeSets;
    ...
    public function __construct(string $exportType = Excel::XLSX,$business_type_id)
    {
        ...
        $this->business_type = collect(b_types());
        $manufactures = Manufature::query()->pluck('name', 'id')->all();
        ...
        $unitlist = Unit::query()->pluck('name', 'id')->all();
        $this->unitlist = collect($unitlist);
        if($this->business_type_id == 5){
            $p_types = ProductType::query()->pluck('name', 'id')->all();
            $this->p_types = collect($p_types);
            $generics = Generic::query()->pluck('name', 'id')->all();
            $this->generics = collect($generics);
        }
       
        
        //dd(b_types());
        $attribute_sets = AttributeSet::query()->get();
        $this->attributeSets = collect($attribute_sets);
        // $taxes = Tax::query()->inRandomOrder()->limit(2)->get();
        if($this->business_type_id == 0){
            $b_type = $this->business_type->count() ? $this->business_type->random() : null;
        }else{
            //dd($this->business_type[$this->business_type_id]);
            $b_type = $this->business_type[$this->business_type_id];
        }
    ...
    {
      //  dd("ss");
        $business_type_id = auth()->user()->business->business_type_id ?? 0;
       


        $rules = [
            'business_type'=>'Name of Dropdown business Type',
