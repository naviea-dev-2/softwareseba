...
class PurchaseReturnReportExport implements  FromArray , WithHeadings , WithStyles, WithCustomStartCell, WithColumnWidths, WithEvents
{
    private $data;
    private $startDate;
    private $endDate;
    ...
    public function __construct($reports , $startDate, $endDate, $company)
    {
        ...
        if($formattedData != [])
        {
            $formattedData[] = [
                ...
                'Product'       =>'',
                'Qty'  =>$total_qty,
                'Unit Price'   =>round($total_per_cost, 2),
                'Discount'  =>round($total_discount, 2),
                'Amount'  =>round($total_sub, 2),
                'Status'  =>'',
            ];
        }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->getDelegate()->setCellValue('A5', 'Purchase Return Report')->getStyle('A5')->getFont()->setBold(true);
                $startRow = 1;
