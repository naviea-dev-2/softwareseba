...
class ProductExpiretExport implements  FromArray , WithHeadings , WithStyles, WithCustomStartCell, WithColumnWidths, WithEvents
{
    private $data;
    private $startDate;
    private $endDate;
    ...
    public function __construct($reports, $company)
    {
        ...
        foreach($reports as $key=>$report)
        {
            ...
            $formattedData[] = [
                ...
                'Product Code'  =>$report->product_code,
                'Batch No'   =>$report->batch_no,
                'Expire Date'  =>date('Y-m-d', strtotime($report->expire_date)),
                'Qty'  =>$report->qty,
            ];
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->getDelegate()->setCellValue('A5', 'Prodcut Expire Report')->getStyle('A5')->getFont()->setBold(true);
                $startRow = 1;
