...
class PurchaseReportExport implements  FromArray , WithHeadings , WithStyles, WithCustomStartCell, WithColumnWidths, WithEvents
{
    private $data;
    private $startDate;
    private $endDate;
    ...
    public function __construct($reports , $startDate, $endDate, $company)
    {
        ...
        foreach($reports as $key=>$report)
        {
            ...
            $formattedData[] = [
                ...
                'Qty'  =>$report->qty,
                'Unit Price'   =>auth()->user()->currency_symbol.round($report->per_cost, 2),
                'Discount'  =>auth()->user()->currency_symbol.round($report->discount, 2),
                'Amount'  =>auth()->user()->currency_symbol.round($report->total, 2),
                'Status'  =>$status,
            ];
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->getDelegate()->setCellValue('A5', 'Purchase Report')->getStyle('A5')->getFont()->setBold(true);
                $startRow = 1;
