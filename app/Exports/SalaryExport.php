    ...
    public function __construct($reports , $startDate, $endDate, $company)
    {
        ...
        foreach($reports as $key=>$report)
        {
            ...
            $formattedData[] = [
                ...
                'Employee Name'=>$report->employee?->employee_name,
                'Employee Id'  =>$report->employee?->employee_id,
                'Net Salary'   => auth()->user()->currency_symbol.' '.round($report->netSalary,2),
                'Paid'         => auth()->user()->currency_symbol.' '.round($report->paidSalary,2),
                'Due'       => auth()->user()->currency_symbol.' '.round($report->due_amount,2),
        ...
        if($formattedData != [])
        {
            $formattedData[] = [
                ...
                'Employee Name'       =>'',
                'Employee Id'  =>'',
                'Net Salary'   => auth()->user()->currency_symbol.round($t_nets, 2),
                'Paid'         => auth()->user()->currency_symbol.round($t_paids, 2),
                'Due'       =>auth()->user()->currency_symbol.round($t_dues, 2),
                'Payment Status'  =>'',
