...
class ProfLossExport implements  FromArray , WithHeadings , WithStyles, WithCustomStartCell, WithColumnWidths, WithEvents
{
    ...
    private $endDate;
    private $company;
    public function __construct($transactions,$direct_tans,$indirect_tans , $startDate, $endDate, $company)
    {
        $formattedData = [];
        $total = 0;
        ...
        if($formattedData != [])
        {
            ...
            if($total_dr > $total_cr){
                $formattedData[] = [
                    ...
                    'Account Code'       =>'',
                    'Type' => '',
                    'Debit'  =>auth()->user()->currency_symbol.round($total_dr-$total_cr,2),
                    'Credit'   =>  ''
                ];
            }else{
                $formattedData[] = [
                    ...
                    'Type' => '',
                    'Debit'  =>'',
                    'Credit'   =>  auth()->user()->currency_symbol.round($total_cr-$total_dr,2)
                ];
            }
        }
