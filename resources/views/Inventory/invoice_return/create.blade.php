...
<script>
    ...
    @foreach (old('product_id') as $k=>$product_id)
        ...
        @php
            $color=App\Models\Inventory\Color::find(old('color')[$k]);
            $size=App\Models\Inventory\Size::find(old('size')[$k]);
            $unit=App\Models\Inventory\Unit::find(old('unit')[$k]);
        @endphp

        var color_option = new Option("{{$color?->name}}","{{$color?->id}}", true, true);
        var size_option = new Option("{{$size?->name}}","{{$size?->id}}", true, true);
        var unit_option = new Option("{{$unit?->name}}","{{$unit?->id}}", true, true);
        FillProductOption("{{$product_id}}",p_row_no,0,"{{ old('qty')[$k] }}",color_option,size_option,unit_option);
        // initalColor(p_row_no);
        // initalSize(p_row_no);
        // initalUnit(p_row_no);
        p_row_no++;
