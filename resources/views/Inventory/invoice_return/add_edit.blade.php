...
<div class="container pt-0" style="box-shadow: 0 0 2px gray;border-top:4px solid gray;margin-top:0px;padding-bottom:30px;padding-left:10px;">
    <h4>Sales Return</h4>
    <form action="{{ route('invoice_return.add_edit',$invoice_return->id) }}" method="post">
        @csrf
        <div class="card shadow border">
            <div class="card-header" style="padding:5px;">
                <h4 class="card-title">
                    <a data-action="collapse">Invoice Information</a>
                </h4>
            ...
            <div class="card-content collapse show">
                <div class="card-body pt-0">
                    ...
                    <div class="row p-1">
                        <div class="col-md-6">
                            <p><strong>Invoice Reference :</strong> {{ $invoice_return->invoice?->reference_no }}</p>
                            <p><strong>Date :</strong> {{ date('d/m/Y',strtotime($invoice_return->invoice?->purchase_date)) }}</p>
                        </div>
                        <div class="col-md-6">
                            <p><strong>Csutomer :</strong> {{ $invoice_return->customer?->name }}</p>
                            <p><strong>Branch :</strong> {{ $invoice_return->branch?->name }} </p>
                        </div>
                    </div>
                    <hr>
                    <div class="row p-1">
                        <div class="col-md-3">
                            ...
                            <input type="text" class=" form-control  datepicker {{ $errors->has('return_date') ? 'is-invalid' : '' }}" value="{{ old('return_date') ?? $invoice_return->return_date }}" name="return_date" autocomplete="off" required >
                        ...
                        <div class="col-md-9">
                            ...
                            <input type="text" class=" form-control {{ $errors->has('return_reason') ? 'is-invalid' : '' }}" name="return_reason" autocomplete="off" required value="{{ old('return_reason') ?? $invoice_return->reason }}">
                ...
                <div class="container-fluid">
                    <table class="order-list table-responsive table-bordered table-sm text-center nowrap" cellspacing="0" width="100%">
                        ...
                        <tbody class="add-ajax-product">
                           @foreach ($invoice_return->invoice->items as $k=>$item)
                           @if($item->product)
                            <tr>
                                @php
                                ...
                                <td>
                                    @php
                                       $return_item = $invoice_return->items->where('product_id',$item->product_id)->where('color_id',$item->color_id)->first();
                                        if($return_item){
                                    ...
                                    <div class="form-group">
                                        ...
                                        <Select class="form-control {{ $errors->has('payment_method') ? 'is-invalid' : '' }}" name="payment_method" id="payment_method">
                                            ...
                                            @foreach ($methods as $method)
                                                <option @if(old('payment_method',$invoice_return->payment_method) == $method->id) selected @endif value="{{ $method->id }}">{{ $method->name }}</option>
                                            @endforeach
                                        </Select>
                                ...
                                {{-- <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <Select class="form-control {{ $errors->has('payment_status') ? 'is-invalid' : '' }}" name="payment_status" id="payment_status">
                                            <option @if(old('payment_status',$invoice_return->payment_status) == 0) selected @endif value="0">Due</option>
                                            <option @if(old('payment_status',$invoice_return->payment_status) == 1) selected @endif value="1">Partial</option>
                                            <option @if(old('payment_status',$invoice_return->payment_status) == 2) selected @endif value="2">Paid</option>
                                        </Select>
