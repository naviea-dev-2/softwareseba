...
<div class="container pt-0" style="box-shadow: 0 0 2px gray;border-top:4px solid gray;margin-top:0px;padding-bottom:30px;padding-left:10px;">
    <h4>Sales Return</h4>
    <form action="{{ route('invoice_return.add',$invoice->id) }}" method="post">
        @csrf
        <div class="card shadow border">
            <div class="card-header" style="padding:5px;">
                <h4 class="card-title">
                    <a data-action="collapse">Sales Information</a>
                </h4>
            ...
            <div class="card-content collapse show">
                <div class="card-body pt-0">
                    ...
                    <div class="row p-1">
                        <div class="col-md-6">
                            <p><strong>Sales Reference :</strong> {{ $invoice->reference_no }}</p>
                            <p><strong>Date :</strong> {{ date('d/m/Y',strtotime($invoice->invoice_date)) }}</p>
                            <p><strong>Area Manager :</strong> {{ $invoice->asr?->employee_name }} </p>
                            <p><strong>Driver :</strong> {{ $invoice->driver?->employee_name }} </p>
                        </div>
                        <div class="col-md-6">
                            <p><strong>Customer :</strong> {{ $invoice->customer?->name }}</p>
                            <p><strong>Branch :</strong> {{ $invoice->branch?->name }} </p>
                            <p><strong>DSR :</strong> {{ $invoice->dsr?->employee_name }} </p>
                    ...
                    <div class="row p-1">
                        <div class="col-md-3">
                            ...
                            <input type="text" class=" form-control  datepicker {{ $errors->has('return_date') ? 'is-invalid' : '' }}" value="{{ old('return_date') ?? date('Y-m-d') }}" name="return_date" autocomplete="off" required >
                        ...
                        <div class="col-md-9">
                            ...
                            <input type="text" class=" form-control {{ $errors->has('return_reason') ? 'is-invalid' : '' }}" name="return_reason" autocomplete="off" required value="{{ old('return_reason') }}">
                        ...
                        <tbody class="add-ajax-product">
                           ...
                           @if($item->product)
                            <tr>
                                ...
                                <td>{{ $item->qty }}</td>
                                <td>
                                    <input data-id="{{ $item->id }}" class="cal_return return_qty_{{$item->id }}" type="number" name="return_qty[{{ $item->id }}]" value="{{ old('return_qty') ? old('return_qty')[$item->id] : 0 }}" max="{{ $item->qty }}">
                                </td>
...
<script>
    ...
    function priceCalculation(paid_o = 0){
        ...
        $('.cal_return ').each(function(){
            ...
            console.log($('.return_tax_'+id).attr('dis-per'));
            if($('.return_tax_'+id).attr('is_per') == 1){
                ...
                var tax = parseFloat($('.return_tax_'+id).attr('dis-per'));
                tax = parseFloat(tax); 
