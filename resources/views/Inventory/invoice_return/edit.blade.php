...
<div class="container pt-0" style="box-shadow: 0 0 2px gray;border-top:4px solid gray;margin-top:0px; padding-bottom:30px;padding-left:10px;">
    ...
    <form action="{{ route('purchase_return.update',$purchase_return->id) }}" method="post">
        @csrf
                ...
                <div class="card-body pt-0">
                    ...
                    <div class="row p-1">
                        <div class="col-md-3">
                            ...
                            <input value="{{ $purchase->vendor?->name }}" type="text" class=" form-control  vendor-auto-search" name="name" autocomplete="off" required>

                        </div>
                        <div class="col-md-3">
                            <label for=""><b>Company Email</b></label>
                            <input value="{{ $purchase->vendor?->email }}" type="email" class=" form-control  vendor-auto-search" name="email" autocomplete="off">
                        </div>
                        <div class="col-md-3">
                            <label for=""><b>Company Mobile</b></label>
                            <input value="{{ $purchase->vendor?->mobile }}" type="number" class=" form-control  vendor-auto-search" name="mobile" autocomplete="off" required>
                        </div>

                        <div class="col-md-3">
                            <label for=""><b>Company Address</b></label>
                            <input value="{{ $purchase->vendor?->address }}" type="text" class=" form-control  vendor-auto-search" name="address" autocomplete="off" required>
                        </div>
                    </div>
                    <div class="row p-1">
                        <div class="col-md-3">
                            <label for=""><b>Purchase Date</b></label>
                            <input type="text" class=" form-control  datepicker" value="{{ date('d-m-Y') }}" value="{{ $purchase->purchase_date }}" name="purchase_date" autocomplete="off" required>
                        </div>
                        <div class="col-sm-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <select name="status" class="form-control  mb-0">
                                <option @if($purchase->status == 1) selected @endif value="1">Recieved</option>
                                <option  @if($purchase->status == 2) selected @endif value="2">Partial</option>
                                <option  @if($purchase->status == 3) selected @endif value="3">Pending</option>
                                <option  @if($purchase->status == 4) selected @endif value="4">Ordered</option>
                            </select>
                    ...
                    <table class="order-list table-responsive table-bordered table-sm text-center nowrap" cellspacing="0" width="100%">
                        ...
                        <tbody class="add-ajax-product">
                            ...
                            <tr id="row_{{ $k+1 }} ss">
                                ...
                                <td>
                                    <input type="hidden" name="old_product_id[{{ $item->id }}]" class="select_product_id_{{ $k+1 }}" value="{{ $item->product_id }}">
                                    <label data-in="{{ $k+1 }}" data-id="{{ $item->product_id }}" class="select_product_code_{{ $k+1 }} select_product_code">{{ $item->product->product_code }}ss</label>
                                </td>
                                <td>
                                    <select data-in="{{ $k+1 }}" class="form-control select_color_{{ $k+1 }} select_color mb-0" name="old_color[{{ $item->id }}]">
                                        <option value="">Color</option>

                                    </select>
                                </td>
                                <td>
                                    <select data-in="{{ $k+1 }}" data-id="0" class="form-control select_size select_size_{{ $k+1 }} mb-0" name="old_size[{{ $item->id }}]">
                                        <option value="">Size</option>
                                    </select>
                                </td>
                                <td>
                                    <input data-in="{{ $k+1 }}" class="form-control touchspin  select_qty_{{ $k+1 }} select_qty" type="number" name="old_qty[{{ $item->id }}]" value="{{ $item->qty }}" placeholder="Qty">
                                </td>
                                <td>
                                    <select name="old_unit[{{ $item->id }}]" data-in="{{ $k+1 }}" class="form-control select_unit_{{ $k+1 }} select_unit mb-0">
                                        <option value="">Unit</option>
                                        <option @if($item->unit == "Piece") selected @endif>Piece</option>
                                        <option @if($item->unit == "Pata") selected @endif>Pata</option>
                                        <option @if($item->unit == "Box") selected @endif>Box</option>
                                        <option @if($item->unit == "Carton") selected @endif>Carton</option>
                                    </select>
                                </td>
                                <td>
                                <input data-in="{{ $k+1 }}" class="form-control  purchase_price_{{ $k+1 }} select_purchase_price" name="old_per_cost[{{ $item->id }}]" value="{{$item->per_cost}}" type="text" placeholder="Price">
                                </td>
                                <td>
                                <input class="form-control  discount_price_{{ $k+1 }} select_discount_price" type="text" value="{{$item->discount}}" name="old_discount[{{ $item->id }}]" value="0" placeholder="Discount">
                                </td>
                                <td>
                                <input name="old_total[{{ $item->id }}]" data-in="{{ $k+1 }}" value="{{$item->total}}" type="text" class="form-control  total_price_{{ $k+1 }} select_total_price" placeholder="Total">
                                </td>
                            </tr>
                            @endforeach
                            <tr id="row_{{$purchase->items->count() + 1}}">

                                <td>
                                    <input type="hidden" name="product_id[]" class="select_product_id_{{$purchase->items->count() + 1}}">
                                    <label data-in="{{$purchase->items->count() + 1}}" data-id="0" class="select_product_code_{{$purchase->items->count() + 1}} select_product_code">Code..</label>
                                </td>
                                <td>
                                    <select data-in="{{$purchase->items->count() + 1}}" class="form-control select_color_{{$purchase->items->count() + 1}} select_color mb-0" name="color[]">
                                        <option value="">Color</option>
                                    </select>
                                </td>
                                <td>
                                    <select data-in="{{$purchase->items->count() + 1}}" data-id="0" class="form-control select_size select_size_{{$purchase->items->count() + 1}} mb-0" name="size[]">
                                        <option value="">Size</option>
                                    </select>
                                </td>
                                <td>
                                    <input data-in="{{$purchase->items->count() + 1}}" class="form-control touchspin  select_qty_{{$purchase->items->count() + 1}} select_qty" type="number" name="qty[]" value="1" placeholder="Qty">
                                </td>
                                <td>
                                    <select name="unit[]" data-in="{{$purchase->items->count() + 1}}" class="form-control select_unit_{{$purchase->items->count() + 1}} select_unit mb-0">
                                        <option value="">Unit</option>
                                        <option>Piece</option>
                                        <option>Pata</option>
                                        <option>Box</option>
                                ...
                                <td>
                                <input data-in="{{$purchase->items->count() + 1}}" class="form-control  purchase_price_{{$purchase->items->count() + 1}} select_purchase_price" name="per_cost[]" type="text" placeholder="Price">
                                </td>
                                <td>
                                <input class="form-control  discount_price_{{$purchase->items->count() + 1}} select_discount_price" type="text" name="discount[]" value="0" placeholder="Discount">
                                </td>
                                <td>
                                <input name="total[]" data-in="{{$purchase->items->count() + 1}}" type="text" class="form-control  total_price_{{$purchase->items->count() + 1}} select_total_price" placeholder="Total">
                                </td>
                            </tr> --}}
                    ...
                    <div class="row mt-3">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <Select class="form-control {{ $errors->has('payment_method') ? 'is-invalid' : '' }}" name="payment_method" id="payment_method">
                                            ...
                                            @foreach ($methods as $method)
                                                <option @if(old('payment_method',$invoice->payment_method) == $method->id) selected @endif value="{{ $method->id }}">{{ $method->name }}</option>
                                            @endforeach
                                        </Select>
                                ...
                                {{-- <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <Select class="form-control {{ $errors->has('payment_status') ? 'is-invalid' : '' }}" name="payment_status" id="payment_status">
                                            <option @if(old('payment_status',$invoice->payment_status) == 0) selected @endif value="0">Due</option>
                                            <option @if(old('payment_status',$invoice->payment_status) == 1) selected @endif value="1">Partial</option>
                                            <option @if(old('payment_status',$invoice->payment_status) == 2) selected @endif value="2">Paid</option>
                                        </Select>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group row mt-1">
                                ...
                                <div class="col-md-6">
                                    <input value="{{ old('shipping_cost') ?? $purchase->order_discount}}" type="number" id="sub_shipping_cost" name="shipping_cost" class="form-control" value="0" />
                                </div>
                             ...
                             <div class="form-group row mt-1">
                                ...
                                <div class="col-md-6">
                                    <input type="number" id="sub_order_discount" name="order_discount" value="{{ old('order_discount') ?? $purchase->order_discount}}" class="form-control" />
                                </div>
                            ...
                            <div class="form-group row mt-1">
                                ...
                                <div class="col-md-6">
                                    <input type="number" id="paid_amount" name="paid_amount" class="form-control" value="{{ old('paid_amount') ?? $purchase->paid_amount }}" />
                                </div>
                            ...
                            <div class="form-group row mt-1">
                                ...
                                <div class="col-md-6">
                                    <input disabled type="number" id="due_amount" name="due_amount" class="form-control" value="{{ old('paid_amount') ?? $purchase->paid_amount }}" />
                                </div>
...
<script>
    ...
    //     var category_option = new Option(product.cat_name, product.cat_id, true, true);
    //    $('.select_cat_'+p_row_no).append(category_option).trigger('change');
    //     var product_option = new Option(product.name+" ("+product.code+")", product.p_id, true, true);
    //    $('.select_product_'+p_row_no).append(product_option).trigger('change');
        // $('input[name="search"]').val(vendor.name);
        $('.auto-search-product').slideUp();
    });
    ...
    @if($purchase->branch_id)
        var branch_option = new Option("{{ $purchase->branch->name }}","{{ $purchase->branch_id }}", true, true);
    ...
    $(document).on('keyup','.select_qty,.select_purchase_price,.select_discount_price,#sub_order_discount,#sub_shipping_cost,#paid_amount,#due_amount',function(){
        priceCalculation();
    });
    ...
    //     var p_id = $('.select_product_code_'+$(this).attr('data-in')).attr('data-id');
    //     console.log(p_id);
    //     if(p_id != 0){
    //         var p_price = 0;
    //         for(var i=0;i<product_price[p_id].length;i++){
    //         // console.log(product_price[i]);
    //             if(product_price[p_id][i].unit_id == $(this).val()){
    //                 p_price = product_price[p_id][i];
    //                 break;
    //             }
    //         }
    //         if(p_price != 0){
    //             $('.purchase_price_'+$(this).attr('data-in')).val(p_price.purchase_price);
    //             priceCalculation();
    //         }else{
    //             $('.purchase_price_'+$(this).attr('data-in')).val(0);
    //              priceCalculation();
    //         }
    //     }
    ...
    function FillProductOption(id,row_no,old=0,qty=1,color_option=null,size_option=null,unit_option=null){
         ...
         $.ajax({
            ...
            success: function(data) {
                if($('.add-ajax-product').find('.pro-row-'+id).length == 0){
                    ...
                    // $('.select_unit_'+row_no).append(unit_option).trigger('change');
                    //  if(color_option){
                    //     $('.select_color_'+row_no).append(color_option).trigger('change');
                    // }
                    // if(size_option){
                    //     $('.select_size_'+row_no).append(size_option).trigger('change');
                    // }
                    // if(unit_option){
                    //     $('.select_unit_'+row_no).append(unit_option).trigger('change');
                    // }
    ...
    @if($purchase->items->count() > 0)
        @foreach($purchase->items as $k=>$item)
            ...
            var color_option = new Option("{{$item->color->name}}","{{$item->color->id}}", true, true);

            var size_option = new Option("{{$item->size->name}}","{{$item->size->id}}", true, true);
            // $('.select_size_'+p_row_no).append(size_option).trigger('change');
            // FillProductOption("{{$item->product_id}}",p_row_no,"{{ $item->id }}",color_option,size_option);
            var unit_option = new Option("{{$item->unit?->name}}","{{$item->unit?->id}}", true, true);
        FillProductOption("{{$item->product_id}}",p_row_no,"{{ $item->id }}","{{ $item->qty }}",color_option,size_option,unit_option);
             p_row_no++;
        @endforeach
    @else
        initalColor(1);
        initalSize(1);
    @endif
    function initalColor(r_no){
        console.log($('.select_product_code_'+r_no).attr('data-id'));
        $(document).find('.select_color_'+r_no).select2({
            ...
            containerCssClass: 'select-sm',
            ajax: {
                url: '{{route('select2.product.color')}}',
                dataType: 'json',
                delay: 250,
                data: function (params) {
                return {
    ...
    function initalSize(r_no){
        $(document).find('.select_size_'+r_no).select2({
            ...
            containerCssClass: 'select-sm',
            ajax: {
                url: '{{route('select2.product.size')}}',
                dataType: 'json',
                delay: 250,
                data: function (params) {
                return {
