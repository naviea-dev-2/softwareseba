                ...
                <div class="row row-card-one">
                    <div class="col-sm-12 ">
                        <div class="row report-title">
                            <h4 class=""><b>Purchase Manage</b></h4>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="container pt-0" style="box-shadow: 0 0 2px gray;border-top:4px solid gray;">
                <div class="row row-card-one my-1">
                    <div class="col-md-12 col-lg-12 col-sm-12">
                        ...
                        <!-- start modal -->
                         @if(can_p('purchase.create'))
                        <!-- Button trigger modal -->
                        <a class="btn btn-primary " href="{{ route('purchase.create') }}"><i class="fa fa-plus"></i> New Purchase</a>
                        @endif
                        @php

                            $p_print= can_p('purchase.print');
                        @endphp

                        @include('Inventory.purchase.show-invoice-details')
                        @include('Inventory.purchase.add-modal-payment')
                        @include('Inventory.purchase.show-payment-modal')
                        <div style="display: none;">
                            @include('Inventory.purchase.payment_print')
                        </div>
...
<script>
    ...
    $(document).on("click",'.print-btn-payment', function(){
        ...
        $('#payment_print_code').html(payment.code);
    ...
    $(".add_payment_data_form").on('submit', function(){
     ...
     $(document).on('click','.payment_show',function(){
        ...
        var id = $(this).attr('data-id');
        $.ajax({
            url: "{{url('purchase-payment-show') }}/"+id,
            method: 'GET',
