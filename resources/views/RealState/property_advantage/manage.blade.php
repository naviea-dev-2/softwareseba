...
<title>Manage Property Advantage</title>
<style>
    label{
        font-size: 1.2rem;
    }
            ...
            <div class="row row-card-one">
                <div class="col-sm-12 ">
                    <div class="row report-title">
                        <h4 class=" "><b>Property Advantage</b> </h4>
                    </div>
                </div>
            </div>
        </div>
                                    ...
                                    <div class="container-fluid">
                                        <div class="row row-card-one">
                                            <div class="col-sm-12">
                                                ...
                                                <form method="POST" action="{{route('property_advantage.store')}}" enctype="multipart/form-data" class="add_data_form">
                                                    @csrf
                                                    <div class="row">
...
<script>
    var d_table = $('#amenity_dataTable').DataTable({
        ...
        "scrollX": true,
        "ajax":{
            "url": "{{ route('property_advantage.ajax') }}",
            "dataType": "json",
            "type": "POST",
            data: function(data){
    ...
    $(document).on("click",'.amenity_data_edit',function(){
        ...
        $.ajax({
            url: "{{route('property_advantage.edit') }}?id=" + id,
            method: 'GET',
    ...
    $(document).on('click','.amenity_del_data',function(){
        ...
        Swal.fire({
		    ...
		    if (result.value) {
                window.location = "{{ url('property-advantage-delete') }}/"+id;
            }
        });
    });
    //add data
