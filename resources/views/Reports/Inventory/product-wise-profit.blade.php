                ...
                <div class="row row-card-one my-4">
                    <div class="col-md-12 col-lg-12 col-sm-12">
                        ...
                        <div class="d-flex justify-content-between">
                            <div></div>
                            <div class="d-flex">
                                @if(can_p('product_wise_profit_report_pdf'))
                                <form action="{{ route('product_wise_profit_report_pdf') }}" method="GET">
                                ...
                                @endif
                                @if(can_p('product_wise_profit_report_print'))
                                <form action="{{ route('product_wise_profit_report_print') }}" method="GET">
                                    <input type="hidden" value="{{ $from_date }}" name="p_from_date" class="p_from_date">
                                    <input type="hidden" value="{{ $to_date }}" name="p_to_date" class="p_to_date">
                                    <input type="hidden" value="{{request()->category}}" name="p_category" class="p_category">
                                    <input type="hidden" value="{{request()->product}}" name="p_product" class="p_product">
                                ...
                                @endif
                                @if(can_p('product_wise_profit_report_excel'))
                                <form action="{{ route('product_wise_profit_report_excel') }}" method="GET">
                                    <input type="hidden" value="{{ $from_date }}" name="p_from_date" class="p_from_date">
                                    <input type="hidden" value="{{ $to_date }}" name="p_to_date" class="p_to_date">
                                    <input type="hidden" value="{{request()->category}}" name="p_category" class="p_category">
                                    <input type="hidden" value="{{request()->product}}" name="p_product" class="p_product">
                        ...
                        <table id="dataTable" class="purchase-list table table-striped table-bordered" style="width:100%">
                          ...
                          <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ auth()->user()->currency_symbol }}{{number_format($sub_sale_price , 2)}}</td>
                                <td>{{ auth()->user()->currency_symbol }}{{number_format($report->purchase_price, 2)}}</td>
                                <td>{{ auth()->user()->currency_symbol }}{{number_format($sub_purchase_price, 2)}}</td>
                                <td>{{ auth()->user()->currency_symbol }}{{number_format($sub_profit, 2)}}</td>
                            ...
                            <tr>
                                ...
                                <td>{{ $total_qty }}</td>
                                <td></td>
                                {{-- <td>{{ auth()->user()->currency_symbol }}{{ round($total_unit_sale,2) }}</td> --}}
                                <td>{{ auth()->user()->currency_symbol }}{{ round($total_sale,2) }}</td>
                                <td></td>
                                {{-- <td>{{ auth()->user()->currency_symbol }}{{ round($total_unit_purchase,2) }}</td> --}}
