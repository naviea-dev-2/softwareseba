...
<div class="content-area">
    <div class="container-fluid mt-2" style="background:#ffffff;dmin-height: 55px;padding: 13px 25px;">
        ...
        <div class="d-flex justify-content-between align-items-center">
            <h5 style="font-size: 0.875rem; margin:0;">Leave Part</h5>
    ...
    <div class="row" style="padding-top: 24px;">
        <div class="col-md-12 col-lg-12 col-sm-12">
            <div class="card">
                <div class="card-body">
                     @php
                        $p_edit = can_p('leavePart.edit');
                        $p_delete = can_p('leavePart.delete');
                    @endphp
                    <table id="dataTable" class="table table-striped table-bordered" style="width:100%">
                        ...
                        <tbody>
                            @foreach($leaveParts as $key=>$leavePart)
                                <tr class="{{$leavePart->id}}">
                                    <td>{{$key+1}}</td>
                                    <td>{{$leavePart->levaePartName}}</td>
                                    <td>{{$leavePart->day}}</td>
                                    <td>
                                        @if($p_edit)
                                        <a class="btn btn-primary" href="javascript:void(0)"  data-token="{{csrf_token()}}" id="leavePartEdit" data-id="{{$leavePart->id}}" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                            <i class="bx bx-edit"></i>
                                        </a>
                                        @endif
                                        @if($p_delete)
                                            <a title="Delete" class="del_hr_data btn btn-danger" data-action="{{route('leavePart.delete',$leavePart->id)}}" data-token="{{csrf_token()}}" data-id="{{$leavePart->id}}"><i class="bx bx-trash"></i></a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
