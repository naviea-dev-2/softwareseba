        ...
        <div class="br-section-wrapper pt-4">
            <div class="row">
                <div class="col-md-4">
                        ...
                        <h6 class="br-section-label text-center mb-1">Add Designation</h6>
                        <div id="create_errors"></div>

                        <div class="form-layout form-layout-4 pt-1 pb-0" style="border: 1px solid;padding: 10px;">

                          <form action="{{ route('storeDesgtData') }}" method="post">
                            @csrf
                            <div class="row">
                              <div class="col-sm-12 mt-2">
                                ...
                                <select name="department_id" id="" class="form-control {{ $errors->has('department_id') ? 'is-invalid' : '' }}">
                                  ...
                                  @foreach ($departments as $cate )
                                    <option @if(old('department_id') == $cate->id) selected @endif value="{{ $cate->id }}">{{ $cate->name }}</option>
                                  @endforeach
                                </select>
                                @error('department_id')
                              ...
                              <div class="col-sm-12 mt-2">
                                ...
                                <input type="text" value="{{ old("name") }}" placeholder="Enter Name" name="name" id="name" class="form-control fl-datepicker @error('name') is-invalid @enderror">
                                @error('name')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                              </div>
                              @if(auth()->user()->business->business_type_id == 15)
                                <div class="col-sm-12 mt-2">
                                  ...
                                  <select name="type" id="" class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}">
                                    <option  @if(old('type') == 0) selected @endif value="0">None</option>
                                    <option  @if(old('type') == 1) selected @endif value="1">DSR</option>
                                  </select>
                                  @error('type')
                                      <div class="invalid-feedback">{{ $message }}</div>
                                  @enderror
