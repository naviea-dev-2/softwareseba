                        ...
                        <tbody>
                            @php $i=1; $total_dr=0; $total_cr=0;@endphp
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: left;">{{$transaction->l_name}}</td>
                                    <td style="text-align: right;">{{ $transaction->type == "debit" ? (auth()->user()->currency_symbol.' '.round($transaction->amount,2)) : '-' }}</td>
                                    <td style="text-align: right;">{{ $transaction->type == "credit" ? (auth()->user()->currency_symbol.' '.round($transaction->amount,2)) : '-' }}</td>
                                </tr>
                                @php $i++; @endphp
                                @endforeach
                                <tr>
                                    <td >Total</td>
                                    <td style="text-align: right;">{{ auth()->user()->currency_symbol }}{{ round( $total_dr,2) }}</td>
                                    <td style="text-align: right;">{{ auth()->user()->currency_symbol }}{{ round( $total_cr,2) }}</td>
                                </tr>
                        </tbody>
                    </table>
                </div><!--/.product-details-->
